...
// M3 수정(classifySentiment 단일화) 후 DISABLED_ 접두사를 제거하면 GREEN.
...
TEST_F(FeedbackAnalyzerTestFixture, DISABLED_Regression_NeutralFilterMismatch_Case1_Gwaenchan) {
...
TEST_F(FeedbackAnalyzerTestFixture, DISABLED_Regression_NeutralFilterMismatch_Case2_GwaenchanInSentence) {
    std::vector<Feedback> data = {Feedback(u8"괜찮한데 배송은 보통이에요")};
    ExpectSentNeutralCountEqualsFilNeutralCount(data);
}

// Case 3: 감정 키워드 없음 → 양쪽 모두 중립 (수정 후에도 통과해야 하는 대조 케이스)
TEST_F(FeedbackAnalyzerTestFixture, DISABLED_Regression_NeutralFilterMismatch_Case3_NoKeywordDefaultsNeutral) {
    std::vector<Feedback> data = {Feedback(u8"오늘 날씨 좋음")};
    ExpectSentNeutralCountEqualsFilNeutralCount(data);
}

// Mixed batch: sent 중립 3건 vs fil(중립) 2건 (현재 버그)
TEST_F(FeedbackAnalyzerTestFixture, DISABLED_Regression_NeutralFilterMismatch) {
